...
class CustomLogger:
    ...
    def _configure_logging(self) -> None:
        ...
        """
        # Перевірка, чи це головний процес
        if current_process().name != "MainProcess":
            return  # Не налаштовувати логування в дочірніх процесах

        logger.remove()

        # Отримуємо поточну директорію і створюємо папку для логів
        log_directory = Path().absolute() / f"log_{Path().absolute().name}"
        log_directory.mkdir(parents=True, exist_ok=True)

        global_log_path = str(log_directory / "global.log")

        # Визначаємо формати логів
        info_format = "{message}"
        custom_format = self._conditional_format

        # Логування INFO в консоль
        ...
        # Логування WARNING і вище в консоль
        self._add_log_handler(
            sink=sys.stdout,
            level="WARNING",
    ...
    def _log(self, level: str, message_keys: Union[List[str], str], *args: Any, **kwargs: Any) -> None:
        ...
        depth = 2  # Фіксована глибина для логів
...
def get_custom_logger(message_file_name="") -> CustomLogger:
    """
